    ...
    private static Maze genBasicMaze(LocalPlayer player, CommandHandler handler, WorldEdit we, Region region, SimpleMazeGenerator gen){
        try{
            ...
            if(handler.containsArgument("torches")){
                int p = 10; //10 torches on 100 blocks
                int he = 2;
                if(handler.containsArgument("p"))
                    p = Integer.parseInt(handler.getArgumentValue("p"));
                if(handler.containsArgument("h"))
                    he = Integer.parseInt(handler.getArgumentValue("h"));
                        ...
                        for(int j = 1; j < region.getLength()-1; j++){
                            if(rnd.nextInt(100) < p && maze.get(i, he -1, j) == airBlock){
                                if(         maze.get(i-1, he-1, j) != airBlock ||
                                            ...
                                            maze.get(i, he-1, j+1) != airBlock){
                                            maze.set(i, he-1, j, new BaseBlock(BlockID.TORCH));
                                }
                            }
                        }
                    }
            ...
            player.printError("Item '" + e.getID() + "' is not allowed!");
        }catch(Exception e){
            player.printError("Failed on generating maze: " + e.getMessage());
        }
        return null;
    }
}
